...
from ..validators import validate_wav_extension
...
class MediaFile(models.Model):
    ...
    scenario = models.ForeignKey(Scenario, on_delete=models.CASCADE, related_name='mediafiles')

# TODO: Need to add repeat, volume, play, mute?? per section 11.6 of spec?
class VocalFile(models.Model):
    filename = models.FileField(upload_to=vocals_upload_to, storage=OverwriteStorage(), validators=[validate_wav_extension])
    title = models.CharField(max_length=256, default='Purr')
    scenario = models.ForeignKey(Scenario, on_delete=models.CASCADE, related_name='vocalfiles')